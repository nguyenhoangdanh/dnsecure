...
export const authService = {
  ...
  /**
   * Secure verification with rate limiting protection
   */
  async verifyRegistration(
    credentials: VerifyRegistration,
  ): Promise<ApiResponse<AuthResponse>> {
    return api.post<AuthResponse>('/auth/verify-registration', credentials);
  },
  ...
  async resendVerificationCode(
    credentials: ResendVerificationCode,
  ): Promise<ApiResponse> {
    return api.post('/auth/resend-verification-code', credentials);
  },
  ...
  /**
   * Send magic link with enhanced security
   */
  ...
  /**
   * Verify magic link token securely
   */
  ...
  /**
   * Refresh the auth token with enhanced error handling
   */
  ...
  async refreshToken(): Promise<ApiResponse<AuthResponse>> {
    try {
      ...
      if (now - lastRefresh < MIN_REFRESH_INTERVAL) {
        console.log(`Service token refresh throttled - last refresh was ${Math.floor((now - lastRefresh)/1000)}s ago`);
        return {
          success: false,
  ...
  async updateUserProfile(
    ...
    return api.patch<User>('/auth/profile', {
      ...
      ...(options.requirePassword
        ? {currentPassword: options.requirePassword}
        : {}),
    });
  },
...
//    */
//   async verifyRegistration(credentials: VerifyRegistration): Promise<ApiResponse<AuthResponse>> {
//     return api.post<AuthResponse>("/auth/verify-registration", credentials);
//   },
...
//    */
//   async resendVerificationCode(credentials: ResendVerificationCode): Promise<ApiResponse> {
//     return api.post("/auth/resend-verification-code", credentials);
//   },
...
//   async setupTwoFactor(): Promise<ApiResponse<{ qrCode: string }>> {
//     return api.post<{ qrCode: string }>("/auth/setup-2fa", {});
//   },

//   async verifyTwoFactorSetup(code: string): Promise<ApiResponse> {
//     return api.post("/auth/verify-2fa-setup", { code });
//   },

//   async verifyTwoFactor(data: VerifyTwoFactorData): Promise<ApiResponse<AuthResponse>> {
//     return api.post<AuthResponse>("/auth/verify-2fa", data);
//   },
